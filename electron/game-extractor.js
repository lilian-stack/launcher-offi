...
function get7zipPath() {
  ...
  if (isDev) {
    ...
    if (process.resourcesPath) {
      const prodPath = path.join(process.resourcesPath, '7zip', '7z.exe')
      log('🔍 Vérification 7zip (prod):', prodPath)
      if (fs.existsSync(prodPath)) {
        log('✅ 7zip trouvé (prod):', prodPath)
        return prodPath
      }
    }
    
    // Chemin alternatif en production
    const altPath = path.join(app.getAppPath(), 'resources', '7zip', '7z.exe')
    log('🔍 Vérification 7zip (alt):', altPath)
    if (fs.existsSync(altPath)) {
      log('✅ 7zip trouvé (alt):', altPath)
      return altPath
    }
  }
...
async function extractWithNode7z(archivePath, outputDir, onProgress = null) {
  try {
    log('📦 Tentative d\'extraction avec node-7z')
    const node7z = await import('node-7z')
    const sevenZipBin = await import('7zip-bin')
    const stream = node7z.extractFull(archivePath, outputDir, {
      $progress: true,
    ...
    return new Promise((resolve, reject) => {
      stream.on('progress', (progress) => {
        ...
        log('📦 Progression:', progress.percent + '%')
      })
      
      stream.on('end', () => {
        log('📦 Extraction réussie avec node-7z')
        resolve(outputDir)
      })
      
      stream.on('error', (err) => {
        errorLog('📦 Erreur node-7z:', err)
        reject(err)
      })
    })
  } catch (err) {
    errorLog('📦 Erreur lors de l\'import de node-7z:', err)
    throw new Error('Impossible d\'extraire l\'archive. Vérifiez que 7zip est installé.')
  }
}
